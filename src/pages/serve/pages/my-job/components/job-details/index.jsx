                    ...
                    {myJobs?.map((job, index) => (
                        <li key={index}>
                            ...
                            <Link
                                ...
                                to={`/mesajlas/${job.request.user.id}`}
                                className="border rounded-md p-5 cursor-pointer block"
                            >
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-x-3">
                                        <span className="h-2 w-2 rounded-full bg-green-600"></span>
                                        <span className="font-medium text-sm">{job.request.user.name}</span>
                                    </div>
                                    <div className="text-xs font-medium text-zinc-500">{formatDate(job.createdAt)}</div>
                                </div>

                                <div className="pt-4 text-sm font-medium">{job.request.serviceName}</div>
                            ...
                            <li
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-x-3">
                                        ...
                                        <span className="font-medium text-sm">{item?.request.user.name}</span>
                                    </div>
                                    <div
